 ...
 *
 * NOTE: API calls to AbabilPay are proxied through local Next.js API routes
 * (/api/ababilpay/intent and /api/ababilpay/settle) to avoid CORS errors.
 * The secret API key lives server-side only.
 */

// These are local proxy routes — no CORS, no API key exposed to browser
const PROXY_INTENT  = '/api/ababilpay/intent';
const PROXY_SETTLE  = '/api/ababilpay/settle';
...
export async function createPaymentIntent(params: {
  ...
  const res = await fetch(PROXY_INTENT, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(params),
  });

  if (!res) throw new Error('Network error: no response from intent proxy.');
  const json = await res.json();
  if (!json.success) {
    throw new Error(json.error?.message || json.error || json.message || 'Failed to create payment intent');
  }
...
export async function getConnectedChainId(): Promise<number> {
  if (typeof window === 'undefined' || !window.ethereum) throw new Error('No Ethereum wallet found.');
...
function generateNonce(): string {
  ...
  if (typeof window !== 'undefined' && window.crypto) {
    ...
    // Basic fallback for non-browser contexts (not expected to run EIP-3009 signing)
...
export async function submitSettlement(
  ...
  const res = await fetch(PROXY_SETTLE, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      intent_id: intentId,
  ...
  if (!res) throw new Error('Network error: no response from settle proxy.');
  const json = await res.json();
  if (!json.success) {
    throw new Error(json.error?.message || json.error || json.message || 'Settlement failed');
  }
